...
Matches and scores are synced automatically every hour via **cron-job.org** calling:

```
POST https://maldinis.app/api/sync/world-cup/
X-Sync-Token: <value from Render dashboard>
```

- Token is generated by Render (`generateValue: true` in `render.yaml`) and stored as `SYNC_TOKEN`.
- To configure cron-job.org: copy `SYNC_TOKEN` from Render → Environment tab and add it as a custom header.
- To regenerate a compromised token: delete and recreate the `SYNC_TOKEN` env var in Render (triggers redeploy), then update the header in cron-job.org.
- The endpoint enforces a 5-minute cooldown to prevent spam from a misconfigured cron.
- Manual sync still works: `python manage.py sync_world_cup` (bypasses the endpoint and token entirely).
